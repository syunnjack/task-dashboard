...
sukima-platform/
  ...
  services/api/           SQLite・施設認証・イベント・通知判定
  services/notifier/      次フェーズ: Push実配信ワーカー
...
- 施設スタッフがゾーン別に「空き・普通・混雑」を更新
- 簡易整理券の待ち組数入力
- ブランド・ゾーンを埋め込んだ読み取り可能なQR生成
- 来場者の匿名チェックイン
- センサーが算出した収容率だけを受け取るデモ
- 通常予測、施設更新、QR、整理券、センサーの重み付き統合
- 鮮度と情報源数に基づく信頼度表示

デモイベントはブラウザのlocalStorageへ最大500件保存し、2時間を超える現在情報は集計対象から外す。共有API接続時は同じ操作をSQLiteへ保存し、施設、ゾーン、セッション、イベント、通知購読、通知ジョブを永続化する。
...
|---|---|---|
| GET | `/health` | 稼働確認 |
| POST | `/v1/session` | 施設IDと秘密鍵から12時間セッションを発行 |
| GET | `/v1/crowd` | 施設・ゾーン別の混雑スコア取得 |
| POST | `/v1/events/checkin` | 匿名チェックイン |
| POST | `/v1/events` | 認証済み施設のスタッフ・整理券・センサー入力 |
| POST | `/v1/subscriptions` | 通知購読と閾値保存 |
| POST | `/v1/notifications/evaluate` | 閾値判定と通知ジョブ作成 |
| POST | `/v1/invites` | スタッフ招待作成（owner限定） |
| POST | `/v1/invites/accept` | 招待受諾とアクセスキー発行 |

`services/api/worker.mjs` が保留中の通知ジョブを Web Push または Expo Push へ配信する。`PUSH_DRY_RUN=false` とVAPID鍵を設定した場合のみ実送信し、恒久エラーまたは4回失敗で終了する。

WebはPushManager購読、アプリはExpo Push Tokenを取得して公開購読エンドポイントへ保存する。購読登録はIP単位で制限し、施設・ゾーンの存在確認と閾値の範囲補正を行う。

秘密鍵とセッショントークンはSHA-256ハッシュのみをDBへ保存する。書き込みはサーバー側で施設とゾーンの所有関係を確認する。匿名チェックインは接続元を不可逆ハッシュ化し、生のIPアドレスを保存しない。公開前にはリバースプロキシでレート制限、HTTPS、秘密鍵管理、監査ログを追加する。

Web管理画面は`VITE_SUKIMA_API_URL`がある場合だけ共有API接続UIを表示する。APIが未設定または一時停止していても端末内モードでデモを継続できる。
